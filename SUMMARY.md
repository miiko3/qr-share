...
- Версии: Android versionCode/versionName, iOS CFBundleShortVersionString и
  CFBundleVersion синхронизированы. Сейчас в репо: **1.9.0 / code 22**.
  История: v1.5.1 (`af7b2b5`), v1.6.0 (`83e65d9`), v1.6.1 (`de8dc00`),
  v1.6.2 (`e575910`), v1.7.0 (`a32f5f5`), v1.8.0 (`c76b82a`), v1.9.0 (`0008f63`).
  Все релизы опубликованы на GitHub Releases. v1.7.0 отдельно не релизили —
  его исправления вошли в v1.8.0.
...
- v1.9.0 (Android): кнопка «Разработчик» в HomeScreen → https://t.me/yetilov;
  ReceiveScreen — кнопка «Сохранить диагностический кадр»: Y-плоскость кадра
  → PNG в `Загрузки/QRFileTransfer/debug/QR_debug_<ts>.png` (через MediaStore
  Downloads для API 29+, иначе прямой файл). Копия кадра делается только по
  нажатию (`wantCapture` AtomicBoolean → `processFrame(image, wantSnapshot)`
  возвращает `packed`; `yuvToBitmap`/`saveDebugFrame` в ReceiveScreen.kt).
  Версия обеих платформ — 1.9.0 (Android code 22 / iOS CFBundleVersion 22).
...
- v1.9.0: кнопка «Разработчик» (Android) + сохранение диагностического кадра
  камеры; релиз опубликован
  (https://github.com/miiko3/qr-share/releases/tag/v1.9.0), оба артефакта на месте.
- Релизы v1.5.1, v1.6.0, v1.6.1, v1.6.2, v1.8.0, v1.9.0.
- Round-trip тест QR (генерация не виновата).

### Active
- Android-сканирование: ждём от пользователя диагностический кадр
  («Сохранить диагностический кадр» на экране приёма) и числа яркости/контраста
  — чтобы понять, что видит камера (чёрный кадр/размытие/поворот).
- iOS-иконки: пользователь сообщил «не применяются» (реальный iPhone, 1.8.0,
  галочка встаёт, иконка не меняется). `.ipa` v1.8.0/v1.9.0 проверен — Info.plist
  содержит CFBundleAlternateIcons (frame-32…42), PNG в корне бандла, без альфы,
  квадратные 235×235. Значит причина на устройстве/в рантайме; следующий шаг —
  показывать в UI `supportsAlternateIcons` и текст ошибки из
  `setAlternateIconName` (сейчас ошибка молча глотается), затем пересборка.

### Blocked
- Причина неработающего Android-QR не подтверждена (нужны диагностические данные).
